...
(function () {
    ...
    var QRDataCapacity = [[],        // version as row, error correction level(L, M, Q, H) as col
                            [17, 14, 11, 7],
                            [32, 26, 20, 14],
                            [53, 42, 32, 24],
                            [78, 62, 46, 34],
    ...
    function getVersion(textLength, errorCorrectionLevel) {            
        for (var version = 1, j = QRErrorCorrectionLevelIndex[errorCorrectionLevel];
                version < QRDataCapacity.length && QRDataCapacity[version][j] < textLength;
                ++version);
        if (version >= QRDataCapacity.length) {
