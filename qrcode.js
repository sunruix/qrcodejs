...
(function () {
    ...
    QRCodeModel.prototype = {
        ...
        makeBlocks: function() {
            ...
            var partition = QRRSBlockTable[(this.version - 1) * 4 + QRErrorCorrectionLevelIndex[this.ecLevel]];
            var dataAnchor = 0;
            for (let i = 0; i < partition.length; i += 3) {
                ...
                for (let j = 0; j < partition[i]; ++j) {
                    ...
                    for (let k = 0; k < m; ++k) {
                        data[k] = this.codewords.words[dataAnchor + k];
                    }
                    data.reverse();
            ...
            for (let j = 0; j < dataLength; ++j) {
                for (let i = 0; i < dataList.length; ++i) {
                    if (dataList[i][j]) cw[index++] = dataList[i][j];
                }
            }
            for (let j = 0; j < ecLength; ++j) {
                for (let i = 0; i < ecList.length; ++i) {
                    if (ecList[i][j]) cw[index++] = ecList[i][j];
                }
            }
            return cw;
        },
        ...
        makeImpl: function(test, maskPattern) {
            ...
            if (this.dataCache == null) {
                ...
                var temp = this.makeBlocks();
                console.log(temp);
            }
            // this.mapData(this.dataCache, maskPattern);
            this.mapData(temp, maskPattern);
        },
